...
def replay(method: Callable) -> None:
    """Display the history of calls for a Cache method.

    Args:
        method (Callable): Bound Cache method to inspect.
    """
    redis_client = method.__self__._redis
    qualname = method.__qualname__

    calls = redis_client.get(qualname)
    calls_count = int(calls.decode("utf-8")) if calls is not None else 0
    print("{} was called {} times:".format(qualname, calls_count))

    inputs = redis_client.lrange("{}:inputs".format(qualname), 0, -1)
    outputs = redis_client.lrange("{}:outputs".format(qualname), 0, -1)

    for call_input, call_output in zip(inputs, outputs):
        input_str = call_input.decode("utf-8")
        output_str = call_output.decode("utf-8")
